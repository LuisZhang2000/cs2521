...
// map a vertex/number to each url
// returns -1 if the given url isn't found
int vertexID (char *str, DLList url_names) {

	int ID = 0;
	//int ID = DLListLength(url_names);
	char *url;

	while (DLListCurrent(url_names) != NULL) {
        url = DLListCurrent(url_names);
		//printf("current url is: %s\n", url);
		//printf("ID is %d\n", ID);
        if (url == NULL) break;
		if (strcmp(str, url) == 0) {
			DLListMoveTo(url_names, 1); // reset 
			return ID;
		}
		DLListMove(url_names, 1);
		ID++;
		//ID--;
    }
	DLListMoveTo(url_names, 1); // reset 
...
char *stringDup(char *word) 
{
	char *dup = malloc(sizeof(char) * (strlen(word) + 1));
	strcpy(dup, word);
	return dup;
}