...
char * normaliseWord(char *str) {
	...
	// remove punctuation marks at the end of word
...
InvertedIndexBST insertNode(InvertedIndexBST n, char *word, char *filename) 
{
	if (n == NULL) {	// if BST is empty, initialise the BST and all its elements
		...
		if (c < 0) {
			...
			if (checkFileList(n, filename) == 1) { 	// check if word is in the same file, if so then don't insert
				return n;
				...
				n->fileList = fileCurr;
			} else if (strcmp(filename, n->fileList->filename) > 0) { // insert somewhere in middle of the list
...
InvertedIndexBST generateInvertedIndex(char *collectionFilename) {
	...
	char *collection = collectionFilename;
	FILE *fp1, *fp2;

	fp1 = fopen(collection, "r");
	if (fp1 == NULL) {
	...
	while (fscanf(fp1, "%s", filename) == 1) {	// read filenames from collection.txt
		fp2 = fopen(filename, "r");
		if (fp2 == NULL) {
			perror("Error while opening file.\n");
			exit(EXIT_FAILURE);
		}
		while (fscanf(fp2, "%s", word) == 1) {		// read words from the open txt file
			invertedIndex = insertNode(invertedIndex, normaliseWord(word), filename);
		}
		fclose(fp2);
	}
